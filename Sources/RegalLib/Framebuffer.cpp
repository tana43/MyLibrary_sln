...
namespace Regal::Graphics
{
        ...
        DXGI_FORMAT format, bool useDepth, bool useStencil)
    {
        ...
        //フォーマットは本来R8G8B8A8でディスプレイ表示に対応しているが、
        //HDRの精度の都合上R32G32B32A32でもOk
        //fogなどの濃度の値はチャンネルが１つで足りるのでそれにも対応出来るようにしている
        ...
        if (useDepth)
        {
            ...
            texture2dDesc.Format = useStencil ? DXGI_FORMAT_R24G8_TYPELESS : DXGI_FORMAT_R32_TYPELESS;
            texture2dDesc.BindFlags = D3D11_BIND_DEPTH_STENCIL | D3D11_BIND_SHADER_RESOURCE;
            hr = device->CreateTexture2D(&texture2dDesc, 0, depthStencilBuffer.GetAddressOf());
            _ASSERT_EXPR(SUCCEEDED(hr), hr_trace(hr));

            D3D11_DEPTH_STENCIL_VIEW_DESC depthStencilViewDesc{};
            depthStencilViewDesc.Format = useStencil ? DXGI_FORMAT_D24_UNORM_S8_UINT : DXGI_FORMAT_D32_FLOAT;
