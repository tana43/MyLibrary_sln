...
namespace Regal::Demo
{
    void DemoScene::CreateResource()
    {
		...
		gltfModel = std::make_unique<Regal::Resource::GltfModel>(graphics.GetDevice(),"./Resources/Models/sa-23_starfury/scene.gltf");
    ...
    void DemoScene::Render(const float& elapsedTime)
    {
		...
		//3D
		{
			...
			static std::vector<Regal::Resource::GltfModel::Node> animatedNodes{gltfModel->nodes};
			static float time{ 0 };
			gltfModel->Animate(0, time += elapsedTime, animatedNodes);
			gltfModel->Render(graphics.GetDeviceContext(), animatedNodes);
